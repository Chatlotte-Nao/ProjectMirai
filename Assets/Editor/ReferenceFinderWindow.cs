...
public class ReferenceFinderWindow : EditorWindow
{
    ...
    //查找资源引用信息
    [MenuItem("Assets/查找资源引用信息 %#&f", false, 25)]
    static void FindRef()
    {
        InitDataIfNeeded();
        OpenWindow();
    ...
    private void OnGUI()
    {
        ...
        if (EditorWindow.focusedWindow == this && Event.current.button == 1 && Event.current.type == EventType.MouseDown)
        {
            Debug.Log("A");
        }
    ...
    public void DrawOptionBar()
    {
        ...
        //刷新数据
        if (GUILayout.Button("刷新数据", toolbarButtonGUIStyle))
        {
            data.CollectDependenciesInfo();
            needUpdateAssetTree = true;
        ...
        isDepend = GUILayout.Toggle(isDepend, isDepend ? "修改模式(Depend)" : "修改模式(Reference)", toolbarButtonGUIStyle, GUILayout.Width(150));
        ...
        needUpdateState = GUILayout.Toggle(needUpdateState, "是否显示更新状态", toolbarButtonGUIStyle);
